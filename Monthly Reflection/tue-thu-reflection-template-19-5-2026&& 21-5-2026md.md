# 🧠 Reflection Check-In — 9XAI Fellowship

> ⏱️ **Estimated time to complete: 10–15 minutes**  
> Fill this out **twice per week** — on **Tuesday** (mid-week) and **Thursday** (end of week).  
> Be honest — the more specific you are, the better we can support you.

---

## 📋 The Basics

**Your Name**: Abdalrhman Dmour 
**Date**: 19-5-2026 && 21-5-2026
**This is my**: ✅ Tuesday check-in ✅ Thursday check-in

---

## 🎯 What I Actually Did Since Last Check-In

> Don't overthink this — just list the concrete things you worked on. Be specific.  
> ❌ Bad: "Worked on the project"  
> ✅ Good: "Built the JWT auth system with bcrypt + 24h token expiry"

### Tasks I completed or made progress on

1. Make changes on the frontend based on the ui/ux team feedback  
2. Work on the persona side with my team 
3. Know more about the system and the requirements of the system
4. work on the persona generator feature on the system with Nizar and Abdullah


### Pick ONE of the tasks above and tell me more

Work on the persona side with my team && Make changes on the frontend

**What tools/technologies did you use?** (be specific — framework names, libraries, APIs)  

So basically I make the changes and do the tasks using vscode with  helping the AI models (Claude sonnet 4.6 + GPT 5.5 mini) 


**Why did you build it THIS way?** (was there another option you considered?)  

No , because we don't have another way to do it in the current way because we don't have time to use another way and try another options. 

but I searched for another options and I chose the way I have mentioned above

### Which project(s) did you contribute to?

**Primary project**: Voc 360 
**Did you contribute to any other project?**  
- [x] No — focused on my main project only  
- [ ] Yes → Which one(s)? _______________

---

## 🔥 The Hardest Problem I Faced

> Think of a moment since your last check-in where something wasn't working and you had to figure it out.  
> If nothing broke — what was the most challenging thing you built?

**What was the problem?**  
SO basically , there where a bad problem I faced during I was working on the system  , so all of us (21 person) are working in one repository  , so when i clone the repo to make my own changes , the version i cloned keep being old one not the new one , so I have showed this problem to my teammates and i have searched for any solution to solve this problem , so nothing solve this problem , so until now I don't know how to fix this problem.  

**How did you discover it?**
- [x] I found it myself while working
- [ ] A teammate told me about it
- [ ] It came up in testing
- [ ] The program manager / mentor pointed it out
- [ ] A user / demo found it
- [ ] Other: _______________

**Walk me through what you did to fix it** (step by step — like you're explaining to a friend)  
I have searched for any solution to solve this problem , so nothing solve this problem , so until now I don't know how to fix this problem, but I still working on it and I think I will find a solution soon.

**How long did it take?**  
- [ ] Under 1 hour
- [ ] 1-3 hours  
- [ ] Half a day
- [] More than a day
- [x] Still working on it

**Did the fix prevent it from happening again, or was it a quick patch?**  
- [ ] Permanent fix — it won't happen again
- [ ] Temporary — might need revisiting
- [x] Not sure

---

## 📈 Skill Check

> Be real with yourself. Rate your **current comfort level** (not where you want to be).

| Skill | Comfort Level (1-5) | Did it improve since last check-in? |
|---|:---:|:---:|
| Python / Backend (FastAPI, etc.) | ⬜1 ⬜2 ✅3 ⬜4 ⬜5 | ✅ Yes ⬜ No ⬜ N/A |
| Frontend (React, TypeScript, etc.) | ⬜1 ⬜2 ⬜3 ⬜4 ✅ 5 | ✅ Yes ⬜ No ⬜ N/A |
| AI/ML (RAG, Embeddings, Agents) | ⬜1 ⬜2 ✅3 ⬜4 ⬜5 | ⬜ Yes ✅ No ⬜ N/A |
| DevOps (Docker, Deployment, CI/CD) | ⬜1 ⬜2 ⬜3 ✅4 ⬜5 | ✅ Yes ⬜ No ⬜ N/A |
| Communication & Presenting | ⬜1 ⬜2 ⬜3 ⬜4 ✅5 | ✅ Yes ⬜ No ⬜ N/A |
| Problem Solving & Debugging | ⬜1 ⬜2 ⬜3 ⬜4 ✅5 | ✅ Yes ⬜ No ⬜ N/A |
| Teamwork & Collaboration | ⬜1 ⬜2 ⬜3 ⬜4 ✅5 | ✅ Yes ⬜ No ⬜ N/A |

> **1** = "I'd panic if asked to do this alone"  
> **3** = "I can handle it with some Googling"  
> **5** = "I could teach someone else how to do this"

---

## 🤝 Who I Worked With

> No one builds alone. Tell us about your team interactions.

**Name a teammate you worked closely with. What did you do together?**  
Name: so we have working as 21 person as one person and one team , but I have worked with Nizar and Abdullah more closely
What you did: We have worked on the persona generator feature on the system , and we have divided the tasks among us and we have worked on it together.

**Did you help someone who was stuck? What was the situation?**  
Nizar , I helped him in explaining him the best way to make the tasks , also I helped him in solving one of the errors he faced it in github.

**Did someone help YOU when you were stuck? What happened?**  
Yes , Ezz , he helped me in solving one of the errors I faced it in github and in the system.

**Did you contribute to a group outside your primary project?**  
- [x] No — focused on my main project
- [ ] Yes → Which group? _______________  
  What exactly did you do for them? _______________

---

## 💡 The "Aha!" Moment

> What's one thing you learned since your last check-in that clicked — something you didn't understand before but now you do?  
> Try to explain it like you'd explain it to a friend who's not in the program.

So the Aha moment was all my friends and me , we can't solve my own problem about the cloning repo , so I was shocked.  

---

## 🚧 What's Blocking Me

> Be honest. If nothing is blocking you, write "Nothing — I'm clear."

**Is anything slowing you down right now?**  
Yes , Git Hub , I need to know more about it.

**What have you already tried to unblock yourself?**  
I searched for any solution to solve this problem , but nothing solve this problem , so I think I will find a solution soon.

**What would help you move faster?**  
- [x] More time
- [ ] Pair-programming with a specific teammate
- [ ] A mentor/coach session on a specific topic
- [ ] Better documentation / clearer requirements
- [ ] Access to tools/accounts/APIs
- [ ] Nothing — I'm good
- [x] Other: I need to know more about Git Hub.

---

## ⚡ Quick Fire Round

> Don't think. Just write the first thing that comes to mind.

**Since last check-in I'm most proud of**: The way I think now, the way I make decisions and the way I approach problems , I didn't think this way before I started the program.  

**The decision I made that had the biggest impact**: The way I chose my team to work on the system , I made a good decision.  

**One thing I wish I handled differently**: The way I handle my problems , I think I need to handle them differently.  

**My energy level right now** (pick one):  
🔴 Low — struggling to stay focused  
🟡 Medium — having good and bad moments  
🟢 High — feeling productive and engaged  

---

# 📌 THURSDAY ONLY — Weekly Goal Review

> ⚠️ **Fill this section ONLY on Thursday.** Skip it on Tuesday.  
> Pull out your **Sunday Weekly Plan** and let's see how the week went.

### How Did I Do Against My Sunday Plan?

> Copy your goals from Sunday's plan and mark the result.

| # | Goal I Set on Sunday | Status | Notes |
|---|---|:---:|---|
| 1 | fully Implementing the Voc 360 project with my team | ⬜ Done ✅ Partial ⬜ Not Started | _______________ |
| 2 | Do final presenttation for ministry about Voc 360 project and sell it for them| ⬜ Done ⬜ Partial ✅ Not Started | _______________ |
| 3 | Helping my team for anything they need in scope to finish the project | ✅ Done ⬜ Partial ⬜ Not Started | _______________ |
| 4 | I should learn more about the project structure and architecture and how it works | ⬜ Done ✅ Partial ⬜ Not Started | _______________ |
| 5 | I should make the next presentation for the ministry about the project| ⬜ Done ⬜ Partial ✅ Not Started | _______________ |

**If something didn't get done — what happened?**  

So we haven't present the project to the ministry yet , also I must know more and more about the structure of the project.

**Was my Sunday plan realistic?**  
- [ ] Yes — I estimated well
- [ ] Too ambitious — I set too many goals
- [ ] Too easy — I finished early and could have done more
- [x] Mixed — some goals were right, others were off

### Week Summary

**Hours I put in this week (approx.)**: 27 hours

**My biggest strength this week**: Know more about how to build the voc 360 project with my team , also I have improved my communication and teamwork skills

**The skill I most need to level up**: To know the deep details of the project and any project

**If I had to give myself a grade this week (A-F)**: C 
**Why?**: Becauase I think that I need more improve on my technical skills 

**What I want to focus on next week** (this feeds into Sunday's plan):  

To enhance on my skills to understand the project more and more 

---

> 📌 **Cadence Reminder:**  
> 🟦 **Sunday** → Fill out the Weekly Plan (set goals)  
> 🟧 **Tuesday** → Fill out this check-in (skip the Thursday section)  
> 🟥 **Thursday** → Fill out this check-in + the Weekly Goal Review section  
>  
> 💬 *"The person who writes the most specific reflection gets the most accurate progress report."*
