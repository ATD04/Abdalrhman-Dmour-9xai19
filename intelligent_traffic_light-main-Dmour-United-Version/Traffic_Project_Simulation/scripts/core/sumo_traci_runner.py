...
class LiveSimulationEngine:
    def __init__(self, config: dict[str, Any], gui: bool = False) -> None:
        ...
        if service_account_path and Path(service_account_path).exists():
            ...
            try:
                try:
                    ...
                    from core.live_support import DetectorCalibrator
        ...
        self.historical_analytics: dict[str, Any] | None = None
        try:
            try:
                from .live_support import DetectorCalibrator, build_detector_historical_analytics
            except ImportError:
                from core.live_support import DetectorCalibrator, build_detector_historical_analytics

            self.historical_analytics = build_detector_historical_analytics(DetectorCalibrator())
        except Exception as exc:  # noqa: BLE001
            logger.warning("Historical detector analytics unavailable: %s", exc)
        ...
        try:
            ...
            self.alert_dispatcher = AlertDispatcher(config)
        except Exception as exc:  # noqa: BLE001
            logger.warning("Alert dispatcher unavailable: %s", exc)
    ...
    def _launch_sumo(self) -> None:
        ...
        # Capture SUMO stderr for diagnostics instead of discarding silently
        sumo_log_path = self.config["paths"].get("app_data", Path("app/data")) / "sumo_stderr.log"
        try:
            sumo_log_path.parent.mkdir(parents=True, exist_ok=True)
            self._sumo_stderr_fh = open(sumo_log_path, "w", encoding="utf-8")  # noqa: SIM115
        except OSError:
            self._sumo_stderr_fh = subprocess.DEVNULL
        self._process = subprocess.Popen(
            cmd,
            cwd=str(LIVE_SCENARIO_ROOT),
            stdout=subprocess.DEVNULL,
            stderr=self._sumo_stderr_fh,
            text=True,
        )
        for _ in range(30):
            try:
    ...
    def _metrics(self, sim_time: float, lane_metrics: dict[str, dict[str, Any]]) -> dict[str, Any]:
        ...
        for direction, route_info in self.route_defs.items():
            ...
            demand_state = self.demand_state["demand"][direction]
            capacity_veh_h = float(demand_state.get("capacity_veh_h", demand_state.get("google_capacity_veh_h", 1.0)))
            storage_capacity = float(demand_state.get("storage_capacity_vehicles", 0.0) or 0.0)
            queue_utilization = (halting / storage_capacity) if storage_capacity > 0 else 0.0
            normal_share = float(google_live.get("normal_share", demand_state.get("normal_share", 0.0)) or 0.0)
            slow_share = float(google_live.get("slow_share", demand_state.get("slow_share", 0.0)) or 0.0)
            jam_share = float(google_live.get("jam_share", demand_state.get("jam_share", 0.0)) or 0.0)
            share_total = max(normal_share + slow_share + jam_share, 0.0001)
            metrics[direction] = {
                ...
                "target_veh_h": float(demand_state["target_veh_h"]),
                "capacity_veh_h": round(capacity_veh_h, 2),
                "saturation_ratio": round(float(demand_state.get("saturation_ratio", 0.0)), 3),
                "observed_saturation_ratio": round(flow_veh_h / max(capacity_veh_h, 1.0), 3),
                "approach_length_m": round(float(demand_state.get("approach_length_m", 0.0)), 1),
                "avg_lane_width_m": round(float(demand_state.get("avg_lane_width_m", 0.0)), 2),
                "free_pct": round(normal_share / share_total * 100.0, 1),
                "slow_pct": round(slow_share / share_total * 100.0, 1),
                "jam_pct": round(jam_share / share_total * 100.0, 1),
                "congested_pct": round((slow_share + jam_share) / share_total * 100.0, 1),
                "queue_utilization_pct": round(clamp(queue_utilization, 0.0, 1.0) * 100.0, 1),
                "storage_used_vehicles": round(float(halting), 1),
                "storage_capacity_vehicles": round(storage_capacity, 1) if storage_capacity else None,
                "storage_remaining_vehicles": round(max(0.0, storage_capacity - halting), 1) if storage_capacity else None,
    ...
    def _insights(self, metrics: dict[str, Any], sim_time: float, vehicle_ids: list[str] | None = None) -> dict[str, Any]:
        ...
        for direction in DIRECTIONS:
            ...
            if metric["queue_vehicles"] >= self.queue_threshold:
                ...
                if sustained_s >= 10.0 and not self._spillback_fired[direction]:
                    ...
                    q_m = metric["queue_m"]
                    severity_level = "CRITICAL" if metric["queue_vehicles"] >= 15 else "HIGH"
                    events.append({
                        "type": "spillback",
            ...
            if google["congestion_level"] in {"heavy", "severe"}:
                sev = "CRITICAL" if google["congestion_level"] == "severe" else "HIGH"
                events.append({
                    "type": "heavy_congestion",
                    "incident_type": "severe_congestion",
                    "severity": sev,
                    "direction": direction,
                    "message": f"Corridor sensors report {google['congestion_level']} inbound conditions from the {direction.replace('bound', '')} side (+{round(curr_delay)}s delay).",
                    ...
                    try:
                        ...
                        if speed_ms < 0.3:
                            ...
                            if stopped_s >= 8.0 and not entry["fired"]:
                                ...
                                events.append({
                                    "type": "abnormal_stop",
                                    "incident_type": "abnormal_stop_vehicle",
                                    "severity": "MEDIUM",
                                    "direction": direction,
                                    "message": (
        ...
        if any(e["type"] == "spillback" for e in events):
            ...
            recommendation = (
                ...
                f"while corridor sensors show the strongest travel-time penalty on the {google_direction.replace('bound', '')} corridor."
            )

        return {
            "dominant_queue_direction": dominant_direction,
            "corridor_delay_direction": google_direction,
    ...
    def _publish_state(self, sim_time: float) -> None:
        ...
        video_incidents: list[dict[str, Any]] = []
        if self.video_processor is not None:
            try:
                video_incidents = self.video_processor.get_incident_events()
            except Exception:  # noqa: BLE001
                video_incidents = []
        # Feed live observations to forecaster for online learning
        if self.forecaster is not None:
            try:
                from datetime import datetime as _dt, timezone as _tz
                _now = _dt.now(_tz.utc).replace(tzinfo=None)
                for direction in DIRECTIONS:
                    flow = metrics[direction].get("flow_veh_h", 0.0)
                    if flow > 0:
                        self.forecaster.append_observation(direction, _now, flow)
            except Exception:  # noqa: BLE001
                pass
        ...
        if self.alert_dispatcher is not None:
            try:
                ...
                for event in insights.get("events", []):
                    ...
                    event_payload["source"] = "insights"
                    alert_candidates.append(event_payload)
                if anomaly_payload and anomaly_payload.get("incidents"):
                    for incident in anomaly_payload["incidents"]:
                        event_payload = dict(incident)
                        event_payload["source"] = "anomaly"
                        alert_candidates.append(event_payload)
                for incident in video_incidents:
                    event_payload = dict(incident)
                    event_payload["source"] = "video_incident"
                    alert_candidates.append(event_payload)
                dispatched_alerts = self.alert_dispatcher.dispatch(
                    alert_candidates,
                    {
                        "wall_time": now_iso(),
        ...
        state = {
            ...
            "alert_dispatch": self.alert_dispatcher.describe() if self.alert_dispatcher is not None else None,
            "dispatched_alerts": dispatched_alerts,
            "google_error": self._latest_google_error,
            "data_provenance": {
                "google_delay_s": "Live corridor travel-time delta versus free-flow baseline.",
                "google_speed_kmh": "Live corridor speed derived from approach distance and travel time.",
                "target_veh_h": "Simulation demand estimate derived from corridor pressure and a geometry-scaled approach capacity.",
                "capacity_veh_h": "Per-approach capacity scaled from monitored lane count, average lane width, and approach length.",
                "saturation_ratio": "Demand divided by geometry-scaled approach capacity (1.0 = fully saturated).",
                ...
                "congested_pct": "Share of monitored corridor intervals marked slow or jammed.",
                "queue_utilization_pct": "Stopped vehicles divided by geometry-derived approach storage capacity.",
            },
        }
        history_point = {
            ...
            "queue_total_m": insights["total_queue_m"],
            "avg_network_speed_kmh": insights["avg_network_speed_kmh"],
            "network_avg_speed_kmh": insights["avg_network_speed_kmh"],
            "dominant_queue_direction": insights["dominant_queue_direction"],
            "phase_label": signal_plan.get("phase_label"),
            "active_directions": signal_plan.get("active_directions", []),
            "cycle_length_s": signal_plan.get("cycle_length_s"),
            "adaptive_active": self.adaptive_active,
            "source": self.traffic_snapshot.get("source"),
            "events": insights.get("events", []),
            "anomaly_incidents": (anomaly_payload or {}).get("incidents", []) if anomaly_payload else [],
            "video_incidents": video_incidents,
            "per_direction": {
                direction: {
                    ...
                    "google_delay_s": google_summary.get(direction, {}).get("delay_s", 0.0),
                }
                for direction in DIRECTIONS
            },
        }
    ...
    def stop(self) -> None:
        ...
        if self._process and self._process.poll() is None:
            ...
            try:
                ...
                self._process.kill()
        # Close SUMO stderr log file handle
        if hasattr(self, "_sumo_stderr_fh") and hasattr(self._sumo_stderr_fh, "close"):
            try:
                self._sumo_stderr_fh.close()
            except Exception:  # noqa: BLE001
                pass
    ...
    def get_historical_analytics(self) -> dict[str, Any] | None:
        return self.historical_analytics

    def get_peak_hours(self) -> dict[str, Any] | None:
        if not self.historical_analytics:
            return None
        return {
            "generated_at": self.historical_analytics.get("generated_at"),
            "source": self.historical_analytics.get("source"),
            "weekdays": self.historical_analytics.get("weekdays", []),
            "directions": {
                direction: {
                    "top_hours": data.get("top_hours", []),
                    "hourly_profile": data.get("hourly_profile", []),
                }
                for direction, data in self.historical_analytics.get("directions", {}).items()
            },
            "approaches": {
                approach_id: {
                    "top_hours": data.get("top_hours", []),
                ...
                for approach_id, data in self.historical_analytics.get("approaches", {}).items()
            },
        }
