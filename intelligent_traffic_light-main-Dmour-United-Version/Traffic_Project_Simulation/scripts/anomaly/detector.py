...
class AnomalyDetector:
    ...
    def score(
        ...
        # Cross-validate with video events — boost confidence when corroborated
        if video_events:
            self._fuse_video_events(results, video_events)

        anomalies = [r for r in results.values() if r.is_anomaly]

        # Classify incidents with severity levels
        incident_classifications = self._classify_incidents(results, metrics, google_snapshot)
        ...
        return {
            ...
            "directions": {
                d: {
                    ...
                    "reason": r.reason,
                    "severity": self._severity_for_score(r.score),
                    "metrics": r.metric_snapshot,
                }
                for d, r in results.items()
            },
            "incidents": incident_classifications,
            "summary": (
    ...
    def _fuse_video_events(
        self,
        results: dict[str, AnomalyResult],
        video_events: list[dict[str, Any]],
    ) -> None:
        """Cross-validate anomaly scores with YOLO video events.

        When a video event (queue_pressure, abnormal_stopping) aligns with
        a statistical/ML anomaly in the same direction, boost the confidence.
        """
        video_directions: set[str] = set()
        for event in video_events:
            etype = event.get("event_type", "")
            zone = event.get("zone", "")
            if etype in ("queue_pressure", "abnormal_stopping", "incident_crash", "fire"):
                # Map zone to direction
                for direction in DIRECTIONS:
                    if direction.replace("bound", "") in zone:
                        video_directions.add(direction)
                if "junction_core" in zone:
                    video_directions.update(DIRECTIONS)

        for direction in video_directions:
            if direction in results:
                r = results[direction]
                # Boost score when video corroborates detector anomaly
                boosted_score = min(1.0, r.score + 0.15)
                results[direction] = AnomalyResult(
                    direction=r.direction,
                    score=boosted_score,
                    is_anomaly=boosted_score >= self.threshold,
                    reason=f"{r.reason} (corroborated by video event)",
    ...
    def _classify_incidents(
        ...
        """Classify detected anomalies into incident types with operator recommendations."""
        ...
        for direction, r in results.items():
            ...
            metric = metrics.get(direction, {})
            google = google_snapshot.get(direction, {})
            queue = float(metric.get("queue_vehicles", 0))
            speed = float(metric.get("avg_speed_kmh", 0))
            delay = float(google.get("delay_s", 0))

            # Classify by dominant anomaly signal
            if queue >= 15 and speed < 5:
                incident_type = "queue_spillback"
                recommendation = f"PRIORITY: Extend green on {direction} immediately. Queue is blocking junction discharge."
            elif delay > 90 and speed < 10:
                incident_type = "severe_congestion"
                recommendation = f"Heavy congestion on {direction} corridor. Consider upstream metering."
            elif speed < 3 and queue < 5:
                incident_type = "abnormal_stop_vehicle"
                recommendation = f"Possible stalled vehicle on {direction}. Dispatch inspection."
            elif queue >= 10:
                incident_type = "rear_end_risk"
                recommendation = f"High queue density on {direction}. Monitor for rear-end collision risk."
            else:
                incident_type = "general_anomaly"
                recommendation = f"Unusual traffic pattern on {direction}. Monitor situation."
