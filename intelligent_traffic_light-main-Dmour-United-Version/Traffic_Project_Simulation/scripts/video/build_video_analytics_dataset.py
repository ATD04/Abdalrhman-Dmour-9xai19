...
try:
    from ..cli.zone_support import ZoneRepository, zone_for_point
except ImportError:
    from cli.zone_support import ZoneRepository, zone_for_point


def _select_device() -> str:
    """Auto-detect best inference device: MPS (Apple Silicon) > CUDA > CPU."""
...
DEFAULT_MODEL_CANDIDATES: list[Path | str] = [
    SIM_ROOT / "models" / "yolo26x.pt",
    SIM_ROOT / "models" / "yolo26n.pt",
...
try:
    ZONE_DEFINITIONS = ZoneRepository().list_zones("*")
    for _zone in ZONE_DEFINITIONS:
        ZONE_LABELS.setdefault(_zone["zone_id"], _zone.get("label", _zone["zone_id"]))
...
def load_model(model_arg: str | None, use_segmentation: bool = True) -> tuple[YOLO, str]:
    candidates: list[Path | str] = []
    if model_arg:
        candidates.append(model_arg)
    
    # Try segmentation model first if requested
    if use_segmentation:
        candidates.extend([
            SIM_ROOT / "models" / "yolo11x-seg.pt",
            "yolo11x-seg.pt",  # auto-download from hub
        ])
    ...
    for candidate in candidates:
        try:
            ...
            print(f"  Loaded model: {name} (segmentation: {use_segmentation})")
            return model, name
...
def assign_zone(cx: float, cy: float) -> str:
    zone = zone_for_point(cx, cy, ZONE_DEFINITIONS) if ZONE_DEFINITIONS else None
    if zone:
...
def extract_segmentation_metrics(mask: np.ndarray, cx: float, cy: float, 
                                 frame_w: int, frame_h: int) -> dict[str, float]:
    """Extract precise metrics from a vehicle segmentation mask.
    
    Returns:
        - area_pixels: total pixels occupied
        - width_pixels: bounding width
        - height_pixels: bounding height
        - aspect_ratio: width / height
        - occupancy_ratio: how much of frame does this vehicle occupy
    """
    ...
    area_pixels = float(len(coords))
    y_min, x_min = coords.min(axis=0)
    y_max, x_max = coords.max(axis=0)
    width_pixels = float(x_max - x_min + 1)
    height_pixels = float(y_max - y_min + 1)
    aspect_ratio = width_pixels / max(height_pixels, 1.0)
    frame_area = frame_w * frame_h
    occupancy_ratio = area_pixels / max(frame_area, 1.0)
...
def draw_detections_on_frame(
    ...
    """Draw bounding-box + segmentation masks on a BGR frame in-place and return it."""
    # Create overlay for masks blending
    ...
    if masks is not None and hasattr(masks, 'data') and masks.data is not None:
        for i, det in enumerate(detections):
            ...
            try:
                ...
                # Get mask and convert to frame dimensions
                mask_np = masks.data[i].cpu().numpy()
                if mask_np.ndim == 3:
                    mask_np = mask_np[0]  # Handle batch dimension
                
                # Resize mask to match frame dimensions
                mask_np = cv2.resize(mask_np, (frame_w, frame_h), interpolation=cv2.INTER_LINEAR)
                ...
                # Blend mask onto overlay
                cv2.addWeighted(mask_overlay, 0.85, mask_colored, 0.15, 0, mask_overlay)
            except Exception:  # noqa: BLE001
                pass
    
    # Use the mask overlay as the new frame (masks already blended at 15% opacity)
    frame = mask_overlay
    ...
    for i, det in enumerate(detections):
        ...
        # Label background with segmentation area
        seg = det.get("segmentation", {})
        area = seg.get("area_pixels", 0)
        label = f"{class_name} {track_id.split('-')[-1]}  {conf:.0%}  {area:.0f}px"
...
def process_video(
    ...
    model = load_model(model_name, use_segmentation=True)[0]
    ...
    last_masks: Any = None  # last segmentation masks for inter-frame reuse
    ...
    while True:
        ...
        if is_inference_frame:
            ...
            masks = results[0].masks if results and results[0].masks is not None else None
            detections: list[dict[str, Any]] = []
            ...
            if boxes is not None and len(boxes):
                for i in range(len(boxes)):
                    ...
                    if masks is not None and i < len(masks.data):
                        try:
                            mask_np = masks.data[i].cpu().numpy().astype(bool)
                            seg_metrics = extract_segmentation_metrics(
                                mask_np, cx, cy, preview_width, preview_h
                            )
                        except Exception:  # noqa: BLE001
                            pass
            ...
            last_masks = masks  # Save masks for inter-frame reuse
        ...
        # Use last_masks for all frames (from last inference frame)
        draw_detections_on_frame(resized, last_detections, preview_width, preview_h, last_masks)
    ...
    manifest_entry: dict[str, Any] = {
        ...
        "source_path": source_path.name,  # portable: filename only, not absolute path
...
def main() -> None:
    ...
    # Compute representative preview FPS from first video if available
    _representative_preview_fps = None
    if videos:
        _representative_preview_fps = videos[0].get("source_fps")
    manifest   = {
        "generated_at": _dt.now(_tz.utc).isoformat(timespec="seconds"),
        "source_root": str(Path(source_root).name),  # relative, not absolute — portable
        "model_name": model_name,
        "inference_fps": inference_fps,
        "preview_fps": _representative_preview_fps,
        "preview_width": args.preview_width,
        "videos": videos,
